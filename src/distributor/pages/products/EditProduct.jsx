...
export default function EditProduct() {
    ...
    const { data: productData, error: productError } = useSwr(ApiConstant.API_URL + `distributor/products/${params.id}`, fetcher);
    ...
    if (data && subcategoriesData && productData) {
                    ...
                    {({ values, errors, handleChange, handleSubmit, setFieldValue }) => (
                        <form onSubmit={handleSubmit} className='w-full'>
                            ...
                            <div className="my-2">
                                ...
                                <input type="text" onChange={handleChange} name="name" id="name" className="border rounded-md  shadow-md px-4 mt-1 py-2 border-500 w-full outline-none hover: border-gray-400 active:border-gray-600 focus-visible:border-gray-600 active:outline-none focus-visible:outline-none" value={values.name} />
                                ...
                                <select name="category_id" id="category_id" onChange={handleChange} className="mt-1 border px-1 rounded w-full outline-none hover: border-gray-400 py-1 active:border-gray-600 focus-visible:border-gray-600 active:outline-none focus-visible:outline-none">
                                    <option selected={true} disabled={true} > -- Choose Category -- </option>
                                    {
                                        data.data.map((category, index) => {
                                            return (
                                                <option key={index} selected={values.category_id == category.id} value={category.id}>{category.category_name}</option>
                                            )
                                        })
                                    }
                                </select>
                                ...
                                <select name="sub_category_id" id="sub_category_id" onChange={handleChange} className="mt-1 border px-1 rounded w-full outline-none hover: border-gray-400 py-1 active:border-gray-600 focus-visible:border-gray-600 active:outline-none focus-visible:outline-none">
                                    <option selected={true} disabled={true} > -- Choose Sub Category -- </option>
                                    {
                                        subcategoriesData.data.map((subCategory, index) => {
                                            return subCategory.category_id == values.category_id ? <option key={index} selected={values.sub_category_id == subCategory.id} value={subCategory.id}>{subCategory.sub_category_name}</option> : <></>
                            ...
                            <div className="my-2">
                                ...
                                <input type="text" onChange={handleChange} name="price" id="price" className="border rounded-md  shadow-md px-4 mt-1 py-2 border-500 w-full outline-none hover: border-gray-400 active:border-gray-600 focus-visible:border-gray-600 active:outline-none focus-visible:outline-none" value={values.price} />
                            ...
                            <div className="my-2">
                                ...
                                <input type="text" onChange={handleChange} name="moq" id="moq" className="border rounded-md  shadow-md px-4 mt-1 py-2 border-500 w-full outline-none hover: border-gray-400 active:border-gray-600 focus-visible:border-gray-600 active:outline-none focus-visible:outline-none" value={values.moq} />
